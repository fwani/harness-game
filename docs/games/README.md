...
> **완성 순서·완성의 정의는 [ROADMAP.md](ROADMAP.md)가 단일 소스다.** 자동화(planner·dev·qa)는
> ROADMAP의 "현재 대상 게임 1개"를 완성의 정의(DoD)까지 끝낸 뒤 다음 번호로 넘어간다.
> 게임별 규칙·구현 상태 세부는 아래 각 게임 사양 문서를 본다.
