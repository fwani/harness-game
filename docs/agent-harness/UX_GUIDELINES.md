...
> **제품 방향 (2026-06-18): 게임을 번호순으로 한 번에 하나씩 완성한다.** 새 게임 도메인 추가는
> 동결. 단일 소스는 `docs/games/ROADMAP.md` — **현재 대상 게임 1개**를 "완성의 정의(DoD: 싱글
> 완성 + 방 기반 멀티 + 유저 단위 전적 + 문서·QA)"까지 끌어올린 뒤 다음 번호로 넘어간다. planner·
> dev·qa 봇 모두 ROADMAP의 현재 대상 게임에 수렴한다. 멀티 설계는
...
> **0번 원칙 — 게임은 턴제/상호작용이어야 한다 (시뮬레이션 금지).** 클릭 한 번에 판 전체가
> 즉시 계산·종료되면 게임이 아니라 결과 구경이다. 결정이 있는 게임은 매 턴 플레이어가 직접
> 결정하고(한 판을 끝까지 자동 진행하는 `playXxxRound()`는 `deal`/`hit`/`stand`/`nextTurn`
> 같은 step API로 분해), 결정이 없는 즉석 게임은 단계적 공개 + 연속 라운드(누적·선승)로 플레이
> 감각을 준다. 상대(CPU) 차례가 화면에 드러나야 한다. (DoD: `docs/games/ROADMAP.md` A 참조.)
...
- [ ] **턴제/상호작용이다(시뮬레이션 아님)**: 클릭 한 번에 판 전체가 끝나지 않고, 플레이어가
      매 단계 개입한다(결정 게임은 매 턴 입력, 즉석 게임은 단계적 공개 + 연속 라운드). 상대 차례가 보인다.
...
- [ ] **생짜 값 대신 디자인 토큰을 쓴다**(아래 "디자인 토큰" 참조). 공통 색·간격·
      둥글기·그림자는 `var(--*)` 토큰으로, 게임 고유색만 게임 클래스의 로컬 변수로.

## 디자인 토큰 & 시각 일관성

> 게임마다 비주얼이 다른 건 자연스럽다. 문제는 **공통 골격이 없어 생기는 미세한
> 불일치**(둥글기 1px 차, 제각각인 회색 테두리)가 누적돼 "아마추어 같다"는 인상을
> 주는 것이다. 이를 막기 위해 **3-레이어 컨벤션**을 따른다.

### 레이어 구조

1. **토큰** (`src/ui/styles.css`의 `:root`) — 전 게임이 공유하는 단일 소스.
   색·간격·둥글기·그림자·타이포·모션. 새 토큰이 필요하면 여기에 추가한다.
2. **플랫폼 셸 + 공통 클래스** — 로비/헤더(`.app`, `.header-top`)와
   `.game`/`.primary`/`.board`/`.cell` 같은 프리미티브. 게임은 이 위에 변형만 얹는다.
3. **게임 스킨** — 그 게임 고유 비주얼. **위 토큰만** 사용하고, 게임 전용 색은
   게임 클래스에 *로컬 변수*로 정의한다.

### 핵심 규칙 — 생짜 값(magic number) 금지

게임/공통 CSS에서 다음은 직접 쓰지 않고 토큰을 쓴다.

- 공통 색 → `var(--accent)`, `var(--good)`, `var(--bad)`, `var(--border)`, `var(--muted)` …
- 간격(패딩·마진·gap) → `var(--s-1)`~`var(--s-8)` (4px 베이스)
- 둥글기 → `var(--r-sm)`/`--r-md`/`--r-lg`/`--r-full`
- 그림자 → `var(--shadow-1)`, 입체 보드는 `var(--shadow-raise)`
- 글자 크기 → `var(--fs-sm)`~`--fs-xl`

**게임 고유색**은 막지 않되, 골격은 토큰을 공유하도록 게임 클래스에 로컬 변수로 둔다.
**구현 레퍼런스: 보드 게임 그룹**(`styles.css`의 `.board`). `.board`가 스킨 변수
(`--board-bg`/`--board-edge`/`--sq-light`/`--sq-dark`/`--ring-legal`/`--piece-dark`/
`--piece-light`)의 기본값을 정의하고, 격자·셀·돌·기물 골격은 그 변수만 참조한다.
각 게임은 변수만 덮어쓴다.

```css
/* 골격(.board/.cell/.stone)은 공유, 게임은 스킨 변수만 덮어쓴다 */
.board         { --board-bg:#b8860b; --board-edge:#7c5e08; background:var(--board-bg); }
.board.go      { --board-bg:#dcb35c; }
.board.reversi { --board-bg:#1f7a4d; --board-edge:#145138; }
.board.chess   { --board-bg:#6b4f2a; --board-edge:#4a3618; --sq-dark:#8a6a3a; }
.stone.white   { color: var(--piece-light); }   /* 모든 보드 공통 */
```

→ 새 보드 게임 추가 = 스킨 변수 몇 줄. 격자·돌·합법수 링·선택 강조는 자동으로 일관.
선택 강조 링은 의미 토큰 `var(--warn)`을 그대로 쓴다(보드 간 동일).

### 가드(자동 검증)

`npm run lint:design`(= `scripts/agent-harness/agent-verify`에 포함)이
이미 토큰화된 공통 색이 생짜 hex로 되돌아오는 회귀를 막는다. 새 토큰을 도입하면
...
| 장기 (`Janggi.tsx`) | 2인 로컬 / vs CPU 기물 이동 (+ vs CPU 시작 옵션: 내 진영 초/한) | ✅ vs CPU·승부까지 | 모드 토글(2인 로컬/vs CPU). vs CPU는 시작 옵션 폼(`janggiStartOptionsView`, `aria-pressed`·색 비의존 한자+선/후공 라벨)에서 **내 진영(초 선공/한 후공)**을 고른다 — 기본 사람=초. 한(漢)을 고르면 도메인 선착 규칙(초 선착)은 그대로 두고 시작 시 CPU(초)가 곧바로 한 수 둔다(`cpuPlaysFirst`→`startNewGame`). CPU는 사람이 고른 진영의 반대편이며 `chooseCpuJanggiMove`(`janggiCpuView`→`chooseRandomJanggiMove`)로 자동 착수(둘 수 없으면 사람 승리). 선택·합법 수·이동·턴(차례/장군 경고)·외통/포획/빅장 승부 표시는 선택 진영 기준으로 매핑되고, 전적은 사람 진영 승을 a로 보존(`janggiCpuWinSide`, 사람=나/CPU). 진영 구분은 색뿐 아니라 자형(초=이체자)·도형(초 원형/한 각형)·접근성 라벨로도 표시(`janggiView`, 색 비의존) |
...
| 지뢰찾기 (`Minesweeper.tsx`) | 9×9·지뢰 10개 1인 플레이 → 칸 열기(연쇄 공개)·깃발(🚩)·승(클리어)/패(지뢰) | ✅ vs 보드 | `startMinesweeperGame`/`playMinesweeperTurn`/`toggleMinesweeperFlag`(`MathRandomSource` 주입) 연동. 첫 클릭은 안전(첫 칸 `exclude`로 지뢰 미배치), **깃발**: 칸 우클릭(`onContextMenu`·`preventDefault`) 또는 입력 모드 토글(⛏ 열기/🚩 깃발, `aria-pressed`)로 좌클릭·Enter·Space 토글(터치·키보드 접근성). 깃발 칸은 열기 보호(도메인 `revealCell`이 깃발 칸·그 너머 연쇄를 차단, UI도 좌클릭 시 `.error` 사유 안내). **남은 지뢰 = 지뢰 − 깃발**(`remainingMines`→domain `countFlags`) 카운터·남은 안전 칸 표시, 패배 시 모든 지뢰 공개. 미공개/깃발(🚩)/숫자(0~8)/지뢰(💣)를 색뿐 아니라 입체·숫자·기호·라벨로 구분(`minesweeperView.cellView`, `ms-flag`), 상태는 `describeMinesweeperStatus`로 `.outcome` 구분, 좁은 화면 대응(`boardGridStyle`). 종료 후 열기·깃발 모두 차단, 새 게임 리셋(깃발 초기화), 종료 시 전적 저장(`minesweeper`, 승=클리어/패=지뢰, 깃발은 승패 무관). 화면 내 통산 전적·연승 표시(`StreakPanel`) |
...
| 만칼라 (`Mancala.tsx`) | 2인 로컬 / vs CPU 구덩이 클릭 씨 뿌리기(자기 곳간 한 번 더·포획·종료/승부). **시작 옵션**: 구덩이당 씨앗 수(3/4/6 Kalah 변형)·vs CPU 선공/후공 | ✅ vs CPU·승자/무승부까지 | `playMancalaTurn`+`chooseRandomMancalaMove`(`MathRandomSource` 주입) 연동. **시작 옵션 UI**(`mancalaStartOptionsView.ts` 순수 헬퍼: `MANCALA_SEEDS_OPTIONS` 3/4/6·`mancalaFirstPlayerOptions`·`normalizeMancalaStartOptions`·`mancalaHumanPlayer`)로 구덩이당 씨앗 수(`createMancalaBoard(6, seedsPerPit)` config 경로 재사용, 도메인 시그니처 무변경)와 vs CPU 선공/후공을 색 비의존 라벨+`aria-pressed` `<button>`으로 선택(씨앗 수는 두 모드 공통, 선공 선택은 vs CPU에서만). 보드를 CSS 그리드로 렌더(윗줄 P2 구덩이 역순으로 포획 맞은편 세로 정렬, 양 끝 곳간). 자기 차례에 씨앗 있는 구덩이만 `<button>` 활성, 마지막 알이 자기 곳간에 안착하면 같은 플레이어가 한 번 더 둔다(헬퍼 `again` 그대로). vs CPU는 선택한 진영을 사람(=나)이 잡고 사람이 후공이면 시작 시 CPU(player 1)가 곧바로 한 수(한 번 더 포함) 자동 진행. 곳간 점수(`mancalaScoreLabel`)·차례/한 번 더 안내(`mancalaTurnLabel`)·포획 수(`mancalaCaptureLabel`)·승자/무승부(`mancalaOutcomeLabel`)를 표시, 종료 후 입력 차단, 새 게임 리셋, 종료 시 전적 저장(`mancala`, 1=a/2=b/무승부=draw). 구덩이/곳간 소유는 색뿐 아니라 P1/P2·나/CPU 레이블·aria-label(`mancalaPitAriaLabel`/`mancalaStoreAriaLabel`)로 구분(색 비의존), 좁은 화면 대응(`max-width`·가로 스크롤). vs CPU 화면에 통산 전적·연승 표시(`StreakPanel`) |
...
| 원카드 (`OneCard.tsx`) | 무작위 한 판 시작(사람 P0=선·CPU P1·손패 7장) → 버림더미 맨 위와 같은 무늬/숫자 카드 내기 → 낼 게 없으면 한 장 뽑기 → CPU 자동 턴 → 손패 먼저 비우면 승 vs CPU 한 판 | ✅ vs CPU·승자 종료까지 | application `startOneCardGame`/`reshuffleIfNeeded`/`playOneCardCpuTurn`(`MathRandomSource` 주입)과 domain `applyOneCardPlay`/`drawOneCard`/`isLegalOneCardPlay`/`findOneCardWinner`/`topDiscard`만 호출(규칙/합법성/승자/무작위 재구현 금지). 손패는 `<button>`으로 렌더하되 `oneCardView.oneCardHandView`(→`isLegalOneCardPlay`)로 **합법 카드만 활성**, 클릭 시 `applyOneCardPlay`로 낸다. 낼 카드가 0장이면 "한 장 뽑기" `<button>`만 활성→`reshuffleIfNeeded` 후 `drawOneCard`로 회복(불법 내기는 domain throw를 `.error`로, 조용한 무시 금지). 사람 턴 종료 후 CPU 턴은 `oneCardView.playOneCardCpuTurns`로 자동 진행하고 동작 로그(낸 카드/뽑음/패스, `oneCardActionSummary`)를 `role="log"`로 표시. 버림더미 맨 위·손패는 색뿐 아니라 무늬 기호(♠♥♦♣)+숫자+aria-label로 구분(`oneCardCardView`, 색 비의존). 현재 차례(`oneCardTurnLabel`)·손패/더미 수를 `.hint`로 안내, 승자(사람/CPU)를 `oneCardOutcomeLabel`로 `.outcome` 구분, 종료 후 손패/뽑기 비활성 차단, 새 게임 리셋, 종료 시 전적 저장(`onecard`, 사람=a/CPU=b). 손패는 `flex-wrap`으로 좁은 화면 대응. 화면 내 통산 전적·연승 표시(`StreakPanel`) |
| 네모로직 (`Nonogram.tsx`) | 내장 5×5 퍼즐(하트/다이아몬드/플러스/체크무늬) 선택 → 행/열 단서를 보고 칸을 칠하기(■)/X 표시(✕)로 전환 → 칠한 칸이 해답과 정확히 일치하면 클리어 1인 퍼즐 | ✅ vs 퍼즐·클리어까지 | domain `nonogram`을 UI가 직접 호출(난수 없는 결정적 고정 퍼즐이라 별도 application 헬퍼 불필요). `createNonogram`으로 퍼즐 선택·리셋, `markNonogramCell`/`isNonogramSolved`/`nonogramRowClues`/`nonogramColumnClues`만 호출(단서/표시 변경/클리어 재구현 금지). 칸을 `<button>`(키보드 포커스·Enter/Space)으로 노출하고 입력 모드 토글(칠하기/X 표시)로 좌클릭=빈↔칠·빈↔X를 전환, 우클릭은 X 표시 편의 입력. 불법 입력(경계 밖 등)은 조용히 무시하지 않고 `nonogramView.toggleNonogramFill`/`applyNonogramInput`이 전파한 도메인 에러를 `.error`로 표시. 칸은 색뿐 아니라 기호(■/✕/빈)+aria-label(좌표/표시 상태)로 구분(`nonogramCellViews`, 색 비의존), 클리어 시 강조/조작 안내 없이 표시 상태만 기술해 종료-표시 일치. 행 단서는 격자 왼쪽·열 단서는 위(빈 줄=`0`)에 배치(`rowClueLabels`/`columnClueLabels`), 칠한 칸 수(`nonogramFilledLabel`)를 `.hint`로, 진행/클리어를 `describeNonogramStatus`로 `.outcome` 구분, 클리어 후 입력 차단, 퍼즐 변경/새 게임 리셋, 클리어 시 전적 저장(`nonogram`, 승=클리어). 좁은 화면 대응(`nonogram-scroll` 가로 스크롤·`max-width`). 화면 내 통산 전적·연승 표시(`StreakPanel`) |
| 스도쿠 (`Sudoku.tsx`) | 무작위 내장 퍼즐 시작 → 빈 칸 선택 후 숫자패드(1~9)/키보드로 채우기·지우기 → 모든 행·열·3×3 박스가 1~9 충돌 없이 채워지면 클리어 1인 퍼즐 | ✅ vs 퍼즐·클리어까지 | application `startSudokuGame`(`MathRandomSource` 주입)으로 무작위 풀이 가능 퍼즐을 시작하고, `playSudokuPlacement`로 한 칸 채우기/지우기·충돌·클리어 판정을 위임(규칙/난수 재구현 금지). domain `sudokuConflicts`/`isSudokuSolved`/`isSudokuGiven`만 직접 호출(파생 표시용). 9×9 격자를 `<button role=gridcell>`로 노출해 빈 칸 선택→숫자패드(1~9+지우기) 입력, 셀 포커스 시 숫자/Backspace 키로 바로 입력(키보드 조작). 고정 단서 칸은 `disabled`로 편집 불가, 미선택 입력·고정 단서 편집 등 불법/불가 입력은 조용히 무시하지 않고 도메인 throw/사유를 `.error`로 표시. 칸은 색뿐 아니라 굵기(고정 단서)·외곽선(선택)·밑줄+`!` 기호(충돌)와 aria-label(`sudokuView.sudokuCellLabel`: 좌표·고정 단서/입력/충돌 텍스트 병행)로 구분(색 비의존), 3×3 박스 경계는 굵은 선으로 구분. 채운 칸/남은 칸/충돌 수(`sudokuProgressLabel`)를 `.hint`로 항상 표시, 진행/클리어를 `sudokuStatusMessage`로 `.outcome` 구분, 클리어 후 입력 차단, "새 게임"으로 새 무작위 퍼즐 리셋, 클리어 시 전적 저장(`sudoku`, 승=클리어). 좁은 화면 대응(`boardGridStyle`·`num-pad` 줄바꿈). 화면 내 통산 전적·연승 표시(`StreakPanel`) |
| 비나이로 (`Binairo.tsx`) | 무작위 내장 6×6 퍼즐 시작 → 칸 클릭(또는 0/1 키)으로 빈 칸 → 0 → 1 → 빈 칸 순환 입력 → 가로·세로 같은 값 3연속 금지·각 행·열 0/1 동수·모든 행·열 서로 다름을 만족하면 클리어 1인 퍼즐 | ✅ vs 퍼즐·클리어까지 | application `startBinairoGame`(`MathRandomSource` 주입)으로 무작위 퍼즐을 시작하고, `playBinairoPlacement`로 한 칸 채우기/지우기·위반·클리어 판정을 위임(규칙/난수 재구현 금지). domain `binairoViolations`/`isBinairoSolved`/`isBinairoGiven`만 직접 호출(파생 표시용). N×N 격자를 `<button role=gridcell>`로 노출해 클릭 시 `binairoView.nextBinairoValue`로 빈 칸→0→1→빈 칸 순환, 셀 포커스 시 0/1/Backspace 키로 직접 입력(키보드 조작). 고정 단서 칸은 `disabled`로 편집 불가, 고정 단서 편집 등 불법 입력은 조용히 무시하지 않고 도메인 throw 사유를 `.error`로 표시. 칸은 색뿐 아니라 굵기(고정 단서)·밑줄+`!` 기호(위반)와 aria-label(`binairoView.binairoCellLabel`: 좌표·고정 단서/입력/빈 칸/위반 텍스트 병행)로 구분(`binairoCellViews`, 색 비의존). 채운 칸/남은 칸/위반 수(`binairoProgressLabel`)를 `.hint`로 항상 표시, 진행/클리어를 `binairoStatusMessage`로 `.outcome` 구분, 클리어 후 입력 차단, "새 게임"으로 새 무작위 퍼즐 리셋, 클리어 시 전적 저장(`binairo`, 승=클리어). 좁은 화면 대응(`boardGridStyle`). 화면 내 통산 전적·연승 표시(`StreakPanel`) |
| 관전 (`SelfPlay.tsx`) | 보드 게임 선택 → CPU vs CPU 한 판 자동 진행 → 결과 | ✅ 종료·승자/무승부까지 | `playEngineGame`+엔진 어댑터(`createGomokuEngine`/`createGoEngine`/`createReversiEngine`/`createJanggiEngine`/`createConnectFourEngine`/`createTicTacToeEngine`/`createDotsAndBoxesEngine`/`createCheckersEngine`/`createMancalaEngine`/`createNimEngine`/`createChessEngine`)+`chooseRandom*Move`(도트 앤 박스는 `chooseRandomDotsEdge`, 체커는 `chooseRandomCheckersMove`, 만칼라는 `chooseRandomMancalaMove`, 님은 `chooseRandomNimMove`, 체스는 `chooseRandomChessMove`)를 `selfPlayView`로 묶어 호출. 오목/바둑/오델로/장기/커넥트포/틱택토/도트앤박스/체커/만칼라/님/체스 지원, 승자(게임별 side 라벨: 흑/백·초/한·1P●/2P○·X/O·1P/2P·1P(선)/2P(후)·백(선)/흑(후))·무승부(스테일메이트 포함)·수순 길이·최종 보드 요약 표시, 다시 돌리기 회복 경로. 최종 보드는 `selfPlayGlyphBoard`로 색 비의존 글리프(●/○·X/O) 렌더, 장기는 `janggiView` 기물 자형/도형/접근성 라벨로 렌더, 도트 앤 박스는 `selfPlayDotsBoard`로 보드를 뽑아 `dotsAndBoxesView`의 점·변·박스 격자(`dotsGridCells`/`dotsGridTemplate`)·점수(`dotsScoreLabel`)로 렌더하고 박스 소유는 P1/P2 라벨로 색 비의존 구분, 체커는 `selfPlayCheckersBoard`로 뽑아 `checkersView.checkersCellView`(●/○·♚/♔ + 라벨)로 색 비의존 렌더, 만칼라는 `selfPlayMancalaBoard`로 뽑아 `mancalaView`의 점수·구덩이/곳간 라벨(P1/P2, `mancalaScoreLabel`/`mancalaPitAriaLabel`/`mancalaStoreAriaLabel`)로 색 비의존 렌더, 님은 2D 격자가 아니라 더미(piles) 게임이라 `selfPlayNimBoard`로 최종 더미를 뽑아 `nimView`의 더미별 글리프(`nimPileLabel`/`nimStonesSymbol`/`nimPileAriaLabel`, ●●● + 돌 수)로 색 비의존 렌더(종국=모든 더미 0), 체스는 `selfPlayChessState`로 종국 상태를 뽑아 `chessView.chessSquareView`(백=외곽선/흑=채움 자형 + 좌표/기물명 aria-label)로 색 비의존 렌더. 수 제한(무작위 무한 진행) 도달 시 "무종국(수 제한 도달)"으로 우아하게 안내(장기·체커는 king 회피로, 체스는 무작위로는 외통이 드물어 길어질 수 있어 `maxMoves` 상한). 커넥트포(≤42수)·틱택토(≤9수)·도트앤박스(유한 변)·만칼라(씨앗 단조 감소)·님(돌 단조 감소)은 유한 게임이라 수 제한 불필요 |
...
- ✅ ~~원카드(One Card) 플레이 UI 연동~~: 도메인(`oneCard`) + application(`playOneCard`:
  `startOneCardGame`/`reshuffleIfNeeded`/`playOneCardCpuTurn`)만 있던 게임을 `OneCard.tsx`로 연결 —
  사람 vs CPU 한 판(합법 카드만 활성 손패 `<button>`·낼 게 없으면 한 장 뽑기 회복·CPU 자동 턴 로그·
  버림더미 맨 위/손패 색 비의존(♠♥♦♣+숫자)·승자 종료·새 게임·전적 저장(`onecard`))까지
  (완료, `oneCardView` + `oneCardView.test`).
- ✅ ~~네모로직(Nonogram·Picross) 플레이 UI 연동~~: 도메인(`nonogram`)만 있던 결정적 1인 퍼즐을
  `Nonogram.tsx`로 연결 — 내장 5×5 퍼즐 선택·행/열 단서 표시·칠하기/X 표시 모드 토글(빈↔칠/빈↔X,
  우클릭 X 보조)·불법 입력 사유(`.error`)·칠한 칸 수·클리어 판정·퍼즐 변경/새 게임·전적 저장
  (`nonogram`)까지(완료, `nonogramView` + `nonogramView.test`).
- ✅ ~~스도쿠(Sudoku) 플레이 UI 연동~~: 도메인(`sudoku`)·application(`playSudoku`)만 있던 결정적
...
- ✅ ~~비나이로(Binairo·Takuzu) 플레이 UI 연동~~: 도메인(`binairo`)·application(`playBinairo`)만 있던
  결정적 1인 퍼즐을 `Binairo.tsx`로 연결 — 무작위 퍼즐 시작(`startBinairoGame`)·셀 클릭/키보드로
  빈 칸→0→1→빈 칸 순환 입력(`playBinairoPlacement`)·고정 단서 비활성·위반 강조(밑줄+`!`, 색 비의존)·
  불법 입력 사유(`.error`)·진행 요약·클리어 판정·새 게임·전적 저장(`binairo`)까지(완료,
  `binairoView` + `binairoView.test`).
