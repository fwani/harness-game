...
```css
/* 보드 색만 갈아끼우고 격자·셀 골격은 공통 */
.go      { --board-bg: #d9a441; --grid: rgba(0,0,0,.5); }
.othello { --board-bg: #1f7a4d; --grid: rgba(0,0,0,.4); }
.cell    { background: var(--board-bg); border-radius: var(--r-sm); }
```

→ 새 게임 추가 = 팔레트 몇 줄. 일관성은 공짜로 따라온다.
...
> **마이그레이션 현황(2026-06-18):** 공통 테두리/표면/텍스트/강조색은 토큰화 완료.
> 게임별 보드·카드 팔레트(`#1f2937` 등 다수)는 아직 생짜 hex로 남아 있어 점진적으로
> 게임 그룹 단위 토큰화 대상이다(가드는 아직 이들을 막지 않는다).
