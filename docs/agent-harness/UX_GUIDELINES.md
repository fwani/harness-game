...
| 관전 (`SelfPlay.tsx`) | 보드 게임 선택 → CPU vs CPU 한 판 자동 진행 → 결과 | ✅ 종료·승자/무승부까지 | `playEngineGame`+엔진 어댑터(`createGomokuEngine`/`createGoEngine`/`createReversiEngine`/`createJanggiEngine`/`createConnectFourEngine`/`createTicTacToeEngine`)+`chooseRandom*Move`를 `selfPlayView`로 묶어 호출. 오목/바둑/오델로/장기/커넥트포/틱택토 지원, 승자(게임별 side 라벨: 흑/백·초/한·1P●/2P○·X/O)·무승부·수순 길이·최종 보드 요약 표시, 다시 돌리기 회복 경로. 최종 보드는 `selfPlayGlyphBoard`로 색 비의존 글리프(●/○·X/O) 렌더, 장기는 `janggiView` 기물 자형/도형/접근성 라벨로 렌더하고 수 제한(무작위 무한 진행) 도달 시 "무종국(수 제한 도달)"으로 우아하게 안내. 커넥트포(≤42수)·틱택토(≤9수)는 유한 게임이라 수 제한 불필요 |
