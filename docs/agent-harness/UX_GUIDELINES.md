...
## 현재 UI 상태 (관찰: 브라우저 + `src/ui` 코드, 2026-06-16)

| 게임 | 화면 | 플레이 가능성 | UX 메모 |
| --- | --- | --- | --- |
| 가위바위보 (`Rps.tsx`) | 손 선택 → CPU와 1판 | ✅ vs CPU | 라운드 누적 점수 없음(매 판 독립) |
| 홀짝 (`OddEven.tsx`) | 홀/짝 추측 → 추첨 | ✅ vs 난수 | 연승/누적 점수 없음 |
| 카드 딜 (`Deal.tsx`) | 인원·장수 입력 → 딜 | ✅ 딜만 | 게임이 아닌 유틸. 입력 검증 에러 표시 있음 |
| 오목 (`Gomoku.tsx`) | 2인 로컬 착수 | ✅ 승자 판정까지 | 턴/승자 표시·리셋 있음(레퍼런스 구현) |
| 바둑 (`Go.tsx`) | 2인 로컬 착수+따냄 | ⚠️ 부분 | **집 계산·승패 없음**(도메인 `scoreArea` 이슈 #33와 연동 필요) |
| 장기 (`Janggi.tsx`) | 초기 배치 표시 | ❌ **보기 전용** | 도메인 `isLegalMove`/`legalMovesFrom`/`applyMove`, 애플리케이션 `playJanggi`가 **이미 구현**됐는데 UI는 이동 미연동 |
...
- **장기 플레이 UI 연동**: `playJanggi`/`legalMovesFrom`/`applyMove`로 기물 선택·
  합법 수 표시·이동·턴 전환·포획을 화면에 연결한다(현재 보기 전용).
- **바둑 종료·계가 UI**: `scoreArea`(#33) 도메인 완료 후 패스/종료 → 집·승자 표시.
- **기록(record) 노출**: `gameRecord` 도메인 + `GameRecordRepository`가 있으나 어떤
  화면도 대국 기록/히스토리를 보여주지 않는다. 결과 저장·조회 UI가 없다.
- **멀티플레이**: 제품 설명의 "멀티 진행"이 UI에는 로컬 핫시트/대-CPU만 있고
  원격 멀티가 없다. 범위·방식 정의 필요(별도 이슈).
- **누적 점수/세션**: 가위바위보·홀짝의 연승·통산 점수 표시.
