...
> 상태: **설계(design) — 코드 미착수.** 이 문서는 다음 개발부터 적용할 목표 구조와 단계별
> 계획을 정의한다. 각 단계는 독립적으로 머지 가능하고 검증 가능해야 한다.
...
## 사람 결정 필요 (escalate — 추측해 채우지 않는다)

> AGENTS.md "추측하지 않는다" + "에스컬레이션" 기준. 단계 4 착수 **전**에 확정한다.

1. **전송 라이브러리**: 순수 `ws` vs `socket.io` vs 기타. 새 런타임 의존성 → 비용/운영 영향.
2. **기록 영속화 수단**: 인메모리만 / 파일 / DB. DB면 마이그레이션·운영 부담(에스컬레이션 항목).
3. **플레이어 식별/인증**: 익명 세션 토큰 vs 로그인. 보안 정책 변경은 사람 확인 필요(SECURITY.md).
4. **호스팅/배포 형태**: 서버 상주 프로세스 추가 → 인프라/비용 영향.
5. **장기·바둑 종료 판정 의존성**: AI/매치 종료에 장기 외통·바둑 계가가 필요한지(현재 도메인 미구현 범위).
