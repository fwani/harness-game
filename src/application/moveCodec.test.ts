...
describe("parseEngineMove — 형태 호환 스모크 (a: 대표 Move 통과 + 엔진 isLegal에 넘길 수 있음)", () => {
  ...
  it("battleship: { row, col } 사격 좌표가 통과한다", () => {
    const r = parseEngineMove("battleship", { row: 3, col: 7 });
    expect(r).toEqual({ ok: true, move: { row: 3, col: 7 } });
  });

  it("battleship: 형태가 깨진 사격은 malformed_move로 거부", () => {
    expect(parseEngineMove("battleship", { row: 0 })).toEqual({
      ok: false,
      code: "malformed_move",
    });
  });
