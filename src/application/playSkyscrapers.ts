...
// 의존한다. infrastructure/ui 의존 금지. 채우기/지우기·행/열 중복·네 변 가시성(보이는 빌딩 수)
// 단서 위반·클리어 판정 등 규칙은 도메인 함수만 호출하고 재구현하지 않는다. 무작위(어떤 퍼즐로
// 시작할지)는 도메인이 아니라 RandomSource 주입으로 처리한다(다른 라틴방진 헬퍼 — playFutoshiki/
// playKenKen/playSudoku/playBinairo 등 — 와 동일 패턴). UI/전적 저장은 이 모듈 범위 밖이다.
import {
  SKYSCRAPER_PUZZLES,
  createSkyscrapers,
  isSkyscraperSolved,
...
/**
 * 새 마천루 한 판을 시작한다.
 * pickRandomSkyscraperPuzzle로 고른 퍼즐(네 변 단서 + 선택적 고정 단서)을 도메인
 * createSkyscrapers(puzzle)에 위임해 시작 상태를 만든다(검증 중복 금지 — 격자/단서 형식
 * 검증은 도메인이 수행).
 */
 ...
 * - violations: 도메인 skyscraperViolations(next) 결과(행/열 중복 + 변 단서 위반 칸).
