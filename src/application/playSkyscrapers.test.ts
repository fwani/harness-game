...
/** 한 방향으로 본 높이 배열에서 "보이는 빌딩 수"를 센다(테스트용 독립 구현). */
...
/**
 * 백트래킹으로 마천루 퍼즐을 푼다. 행/열 라틴(1..N 한 번씩) + 네 변 가시성 단서를 모두
 * 만족하는 완성 격자(1..N)를 찾으면 반환, 불가능하면 null.
 * (테스트가 "완성 가능"을 실제로 증명하기 위한 독립 솔버 — 프로덕션 코드와 무관.)
 */
function solveSkyscraper(puzzle: SkyscraperPuzzle): number[][] | null {
  const { size } = puzzle;
  ...
  const clue = (
    arr: ReadonlyArray<number | null>,
    i: number,
  ): number | null => arr[i] ?? null;

  const cluesOk = (): boolean => {
    const { clues } = puzzle;
    for (let i = 0; i < size; i += 1) {
      const rowVals = grid[i]!.map((v) => v!) as number[];
      const colVals = grid.map((row) => row[i]!) as number[];
      const left = clue(clues.left, i);
      const right = clue(clues.right, i);
      const top = clue(clues.top, i);
      const bottom = clue(clues.bottom, i);
...
/** 완성 격자를 이용해 빈 칸을 모두 채워 클리어까지 도달한다(playSkyscraperPlacement 사용). */
...
describe("SKYSCRAPER_PUZZLES (도메인 뱅크 소비)", () => {
  it("각 퍼즐은 행/열 라틴 + 네 변 가시성 단서를 만족하게 완성 가능하다(백트래킹 솔버)", () => {
    for (const puzzle of SKYSCRAPER_PUZZLES) {
      const solution = solveSkyscraper(puzzle);
...
describe("pickRandomSkyscraperPuzzle", () => {
  ...
  it("범위 밖/비정수 인덱스를 반환하면 throw 한다", () => {
    ...
    expect(() => pickRandomSkyscraperPuzzle(new FixedRandom([0.5]))).toThrow();
  });
});
...
describe("startSkyscrapersGame", () => {
  ...
  it("같은 퍼즐을 createSkyscrapers로 만든 상태와 동일하게 시작한다(도메인 위임 확인)", () => {
...
describe("playSkyscraperPlacement", () => {
  ...
  it("행/열 중복 입력의 좌표를 violations로 반영한다", () => {
    // 첫 퍼즐 (0,0)=1 고정. (1,0)에 1을 넣으면 같은 열(col 0)에 1이 중복.
    const state = startSkyscrapersGame(new ZeroRandom());
  ...
  it("올바르게 마지막 칸까지 채우면 violations=[]·status=solved 가 된다", () => {
    for (let index = 0; index < SKYSCRAPER_PUZZLES.length; index += 1) {
      ...
      const solution = solveSkyscraper(SKYSCRAPER_PUZZLES[index]!);
      expect(solution).not.toBeNull();
